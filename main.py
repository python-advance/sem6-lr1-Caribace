...
def csv_menwomen(file_obj):
    """
    Количество мужчин и женщин на параходе
    """
...
def csv_port(file_obj):
    """
    Количество пассажиров зашедших на борт в различных портах
    """
    reader = csv.reader(file_obj)
    s, c, q = 0, 0, 0
    for row in reader:
...
def popular_name(file_obj):
    """
    Самое популярное мужское и женское имена людей, старше 15 лет на корабле
    """
    df = pd.read_csv(file_obj)
    reader = df[['Name']]
...
if __name__ == "__main__":
    ...
    with open('train.csv') as f_obj:
         print('6.3) Выясните есть ли корреляция между пассажирским классом, в котором пассажир ехал, и параметром survival;')
